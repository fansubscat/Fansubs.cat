			...
			<div class="card w-100">
				<article class="card-body">
					...
					<table class="table table-hover table-striped">
						...
						<tbody>
<?php
	$result = query("SELECT LOWER(query) query,COUNT(*) cnt,(SELECT COUNT(*) FROM series s WHERE s.name LIKE CONCAT('%',query,'%') OR s.alternate_names LIKE CONCAT('%',query,'%')) results FROM search_history GROUP BY LOWER(query) ORDER BY cnt DESC");
