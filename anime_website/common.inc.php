		...
		while ($vrow = mysqli_fetch_assoc($result)){
			if (!empty($vrow['url'])) {
				...
				if ($vrow['created']>=date('Y-m-d', strtotime("-1 week"))) {
					echo "\t\t\t\t\t\t\t\t\t\t\t\t".'<span class="new-episode'.(in_array($vrow['id'], get_cookie_viewed_links_ids()) ? ' hidden' : '').'" data-link-id="'.$vrow['id'].'" title="Publicat durant la darrera setmana">Novetat!</span>'."\n";
				}
				echo "\t\t\t\t\t\t\t\t\t\t\t\t\t</div>\n";
				echo "\t\t\t\t\t\t\t\t\t\t\t\t".'</td>'."\n";
				echo "\t\t\t\t\t\t\t\t\t\t\t\t".'<td class="right">'."\n";
