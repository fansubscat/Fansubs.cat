...
if (!empty($_GET['query']) || $header_tab=='movies'){
...
switch ($header_tab){
	case 'movies':
		...
		$queries=array(
			$base_query . " WHERE s.type='movie' GROUP BY s.id ORDER BY last_updated DESC LIMIT $max_items",
	...
	case 'series':
		...
		$queries=array(
			$base_query . " WHERE s.type='series' GROUP BY s.id ORDER BY last_updated DESC LIMIT $max_items",
...
LIMIT $max_items");
		...
		$queries=array(
			$base_query . " GROUP BY s.id ORDER BY last_updated DESC LIMIT $max_items",
			$base_query . " WHERE s.id IN ($in_clause) GROUP BY s.id ORDER BY FIELD(s.id,$in_clause)",
			$base_query . " GROUP BY s.id ORDER BY s.air_date DESC LIMIT $max_items",
...
<?php
		while ($row = mysqli_fetch_assoc($result)){
?>
							<div<?php echo ($row['status']==4 && empty($_GET['query'])) ? ' class="cancelled'.($carousel[$i] ? '' : '-not-carousel').'"' : ''; ?>>
