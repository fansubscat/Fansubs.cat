...
if (!empty($_GET['link_id']) && is_numeric($_GET['link_id']) && !empty($_GET['action'])) {
	...
	if ($_GET['action']=='close') {
		if (!empty($_GET['time_spent']) && is_numeric($_GET['time_spent']) && $_GET['time_spent']>60 && $_GET['time_spent']<60*60*3) {
		...
		//Else, discard and not even report it: opened and closed in too little time (<1 min), or left open for too long (>3 h)
	} else {
