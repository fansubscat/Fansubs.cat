...
if (flock($lock_pointer, LOCK_EX)) {
	log_action('fetch-news-started', 'News fetcher has started');
	...
	while ($row = mysqli_fetch_assoc($result)){
		echo "Fetching news from fetcher ".$row['fansub_slug']."/".$row['id']."\n";
	...
	log_action('fetch-news-finished', 'News fetcher has finished');
