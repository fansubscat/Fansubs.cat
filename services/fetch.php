...
if (flock($lock_pointer, LOCK_EX)) {
	$result = mysqli_query($db_connection, "SELECT f.*, (SELECT MAX(date) FROM news n WHERE n.fetcher_id=f.id) last_fetched_item_date FROM fetchers f LEFT JOIN fansubs fa ON f.fansub_id=fa.id WHERE f.fetch_type='periodic'".(isset($fansub_to_fetch) ? " AND fansub_id='$fansub_to_fetch'":"")." ORDER BY fetch_type DESC, fa.name ASC, f.url ASC") or die(mysqli_error($db_connection));
