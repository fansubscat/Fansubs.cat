...
if (flock($lock_pointer, LOCK_EX)) {
	mysqli_query($db_connection, "INSERT INTO action_log (action, text, author, date) VALUES ('fetch-news-started','S\'ha iniciat l\'obtenció de notícies', '(Servei intern)', CURRENT_TIMESTAMP)") or die(mysqli_error($db_connection));
	...
	mysqli_query($db_connection, "INSERT INTO action_log (action, text, author, date) VALUES ('fetch-news-finished','S\'ha completat l\'obtenció de notícies', '(Servei intern)', CURRENT_TIMESTAMP)") or die(mysqli_error($db_connection));
