...
query("INSERT INTO recommendation SELECT vr.id FROM version vr LEFT JOIN series mr ON vr.series_id=mr.id WHERE mr.type='manga' AND vr.is_hidden=0 AND (mr.rating<>'XXX' OR mr.rating IS NULL) AND (mr.score>=6 OR mr.score IS NULL OR vr.is_always_featured=1) AND vr.is_featurable=1 AND ((vr.status IN (1,3) AND vr.id NOT IN (".implode(',', $previous_ids).")) OR vr.is_always_featured=1) AND vr.is_missing_episodes=0 ORDER BY vr.is_always_featured DESC, RAND() LIMIT 10");
