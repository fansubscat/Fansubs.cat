	...
	if ($row = mysqli_fetch_assoc($result)) {
		switch($_GET['action']) {
			...
			case 'open':
				...
				query("REPLACE INTO play_session VALUES ('$play_id', $link_id, 0, CURRENT_TIMESTAMP, '".escape($_SERVER['REMOTE_ADDR'])."', '".escape($_SERVER['HTTP_USER_AGENT'])."',NULL)");
			...
			case 'notify':
				...
				if (!empty($_GET['time_spent']) && is_numeric($_GET['time_spent'])){
					...
					query("UPDATE play_session SET link_id=$link_id, time_spent=$time_spent, last_update=CURRENT_TIMESTAMP, ip='".escape($_SERVER['REMOTE_ADDR'])."', user_agent='".escape($_SERVER['HTTP_USER_AGENT'])."', log='".escape($_POST['log'])."' WHERE play_id='$play_id'");
				}
