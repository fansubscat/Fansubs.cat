...
$result = query("SELECT v.*, GROUP_CONCAT(DISTINCT f.name ORDER BY f.name SEPARATOR ' + ') fansub_name FROM version v LEFT JOIN rel_version_fansub vf ON v.id=vf.version_id LEFT JOIN fansub f ON vf.fansub_id=f.id LEFT JOIN series s ON v.series_id=s.id WHERE v.hidden=0 AND v.series_id=".$series['id']."$cookie_extra_conditions GROUP BY v.id ORDER BY v.status ASC, v.created ASC");
...
$related_query="SELECT rs.related_anime_id id, s.name FROM related_anime rs LEFT JOIN series s ON rs.related_anime_id=s.id WHERE (SELECT COUNT(*) FROM version v WHERE v.series_id=s.id AND v.hidden=0)>0 AND rs.anime_id=".$series['id']." UNION SELECT id, NULL FROM series s WHERE (SELECT COUNT(*) FROM version v WHERE v.series_id=s.id AND v.hidden=0)>0 AND id<>".$series['id']." AND author='".escape($series['author'])."' UNION (SELECT series_id id, NULL FROM rel_series_genre sg WHERE (SELECT COUNT(*) FROM version v WHERE v.series_id=sg.series_id AND v.hidden=0)>0 AND series_id<>".$series['id']." GROUP BY series_id HAVING COUNT(CASE WHEN genre_id IN (SELECT genre_id FROM rel_series_genre WHERE series_id=".$series['id'].") THEN 1 END)>=".max($num_of_genres>=2 ? 2 : 1,intval(round($num_of_genres/2))).") ORDER BY name IS NULL ASC, name ASC, RAND() LIMIT $max_items";
...
//1. Related, 2. Same author, 3. Half of genres or more in common
$num_of_genres = count(explode(', ', $series['genres']));
$related_query="SELECT rm.manga_id id, m.name FROM related_manga_anime rm LEFT JOIN manga m ON rm.manga_id=m.id WHERE (SELECT COUNT(*) FROM manga_version v WHERE v.manga_id=m.id AND v.hidden=0)>0 AND rm.anime_id=".$series['id']." UNION SELECT id, NULL FROM manga m WHERE (SELECT COUNT(*) FROM manga_version v WHERE v.manga_id=m.id AND v.hidden=0)>0 AND author='".escape($series['author'])."' UNION (SELECT manga_id id, NULL FROM rel_manga_genre mg WHERE (SELECT COUNT(*) FROM manga_version v WHERE v.manga_id=mg.manga_id AND v.hidden=0)>0 GROUP BY manga_id HAVING COUNT(CASE WHEN genre_id IN (SELECT genre_id FROM rel_series_genre WHERE series_id=".$series['id'].") THEN 1 END)>=".max($num_of_genres>=2 ? 2 : 1,intval(round($num_of_genres/2))).") ORDER BY name IS NULL ASC, name ASC, RAND() LIMIT $max_items";
$resultin = query($related_query);
$in = array(-1);
while ($row = mysqli_fetch_assoc($resultin)) {
	$in[]=$row['id'];
