...
<?php
	...
	$result = query("SELECT v.status, COUNT(v.id) version_count FROM version v LEFT JOIN series s ON v.series_id=s.id WHERE s.type='anime' AND s.has_licensed_parts<=1 GROUP BY v.status ORDER BY status ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		switch ($row['status']) {
			case 5:
...
<?php
	...
	$status_colors=array();
	$status_count_values=array();
	$result = query("SELECT v.status, COUNT(v.id) version_count FROM version v LEFT JOIN series s ON v.series_id=s.id WHERE s.type='manga' AND s.has_licensed_parts<=1 GROUP BY v.status ORDER BY status ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		switch ($row['status']) {
			case 5:
				array_push($status_values, "'".lang('status.cancelled.private.short')."'");
...
<?php
	...
	$status_colors=array();
	$status_count_values=array();
	$result = query("SELECT v.status, COUNT(v.id) version_count FROM version v LEFT JOIN series s ON v.series_id=s.id WHERE s.type='liveaction' AND s.has_licensed_parts<=1 GROUP BY v.status ORDER BY status ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		switch ($row['status']) {
			case 5:
				array_push($status_values, "'".lang('status.cancelled.private.short')."'");
...
<?php
	...
	$status_colors=array();
	$status_count_values=array();
	$result = query("SELECT v.status, COUNT(v.id) version_count FROM version v LEFT JOIN series s ON v.series_id=s.id WHERE s.type='anime' AND s.has_licensed_parts<=1 AND v.id IN (SELECT version_id FROM rel_version_fansub WHERE fansub_id=".$fansub['id'].") GROUP BY v.status ORDER BY status ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		switch ($row['status']) {
			case 5:
				array_push($status_values, "'".lang('status.cancelled.private.short')."'");
...
<?php
	...
	$status_colors=array();
	$status_count_values=array();
	$result = query("SELECT v.status, COUNT(v.id) version_count FROM version v LEFT JOIN series s ON v.series_id=s.id WHERE s.type='manga' AND s.has_licensed_parts<=1 AND v.id IN (SELECT version_id FROM rel_version_fansub WHERE fansub_id=".$fansub['id'].") GROUP BY v.status ORDER BY status ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		switch ($row['status']) {
			case 5:
				array_push($status_values, "'".lang('status.cancelled.private.short')."'");
...
<?php
	...
	$status_colors=array();
	$status_count_values=array();
	$result = query("SELECT v.status, COUNT(v.id) version_count FROM version v LEFT JOIN series s ON v.series_id=s.id WHERE s.type='liveaction' AND s.has_licensed_parts<=1 AND v.id IN (SELECT version_id FROM rel_version_fansub WHERE fansub_id=".$fansub['id'].") GROUP BY v.status ORDER BY status ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		switch ($row['status']) {
			case 5:
				array_push($status_values, "'".lang('status.cancelled.private.short')."'");
