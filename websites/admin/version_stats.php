...
<?php
	$result = query("SELECT IFNULL(SUM(clicks),0) total_clicks, IFNULL(SUM(views),0) total_views, IFNULL(SUM($viewed_content_field),0) total_$viewed_content_field FROM views v LEFT JOIN file f ON v.file_id=f.id WHERE f.version_id=".escape($_GET['id']));
...
?>
					<div class="row">
						...
						<div class="col-sm-4 text-center"><b><?php echo $series_c; ?>:</b> <?php echo $type=='manga' ? $totals['total_'.$viewed_content_field] : get_hours_or_minutes_formatted($totals['total_'.$viewed_content_field]); ?></div>
					</div>
				</article>
			</div>
		</div>
...
<?php
	...
	$result = query("SELECT DATE_FORMAT(v.day,'%Y-%m') month, GREATEST(IFNULL(SUM(clicks),0)-IFNULL(SUM(views),0),0) total_clicks, IFNULL(SUM(views),0) total_views, IFNULL(SUM($viewed_content_field),0)/$viewed_content_divide total_$viewed_content_field FROM views v LEFT JOIN file f ON v.file_id=f.id WHERE f.version_id=".escape($_GET['id'])." GROUP BY DATE_FORMAT(v.day,'%Y-%m') ORDER BY DATE_FORMAT(v.day,'%Y-%m') ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		$months[date("Y-m", strtotime($row['month'].'-01'))]=array($row['total_clicks'], $row['total_views'], $row['total_'.$viewed_content_field]);
	}
	mysqli_free_result($result);
...
<?php
	...
	$result = query("SELECT DATE_FORMAT(v.day,'%Y-%m-%d') day, GREATEST(IFNULL(SUM(clicks),0)-IFNULL(SUM(views),0),0) total_clicks, IFNULL(SUM(views),0) total_views, IFNULL(SUM($viewed_content_field),0)/$viewed_content_divide total_$viewed_content_field FROM views v LEFT JOIN file f ON v.file_id=f.id WHERE f.version_id=".escape($_GET['id'])." GROUP BY DATE_FORMAT(v.day,'%Y-%m-%d') ORDER BY DATE_FORMAT(v.day,'%Y-%m-%d') ASC");
	while ($row = mysqli_fetch_assoc($result)) {
		if (array_key_exists(date("Y-m-d", strtotime($row['day'])), $days)) {
			$days[date("Y-m-d", strtotime($row['day']))]=array($row['total_clicks'], $row['total_views'], $row['total_'.$viewed_content_field]);
		}
	}
	mysqli_free_result($result);
...
?>
							<tr>
								...
								<td class="text-center"><?php echo $type=='manga' ? $row['total_'.$viewed_content_field] : get_hours_or_minutes_formatted($row['total_'.$viewed_content_field]); ?></td>
							</tr>
<?php
	}
?>
