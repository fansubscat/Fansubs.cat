...
$link_id = (!empty($_GET['file_id']) ? intval($_GET['file_id']) : 0);
$play_id = (!empty($_GET['view_id']) ? escape($_GET['view_id']) : '');
...
if ($link_id>0 && !empty($_GET['action']) && !empty($play_id)) {
	...
	if ($row = mysqli_fetch_assoc($result)) {
		switch($_GET['action']) {
			...
			case 'open':
				if ($config['items_type']=='manga') {
					query("REPLACE INTO views SELECT $file_id, '".date('Y-m-d')."', '${config['items_type']}', IFNULL((SELECT clicks+1 FROM views WHERE file_id=$file_id AND day='".date('Y-m-d')."'),1), IFNULL((SELECT views FROM views WHERE file_id=$file_id AND day='".date('Y-m-d')."'),0), NULL, IFNULL((SELECT pages_read FROM views WHERE file_id=$file_id AND day='".date('Y-m-d')."'),0)");
					...
					query("REPLACE INTO views SELECT $file_id, '".date('Y-m-d')."', '${config['items_type']}', IFNULL((SELECT clicks+1 FROM views WHERE file_id=$file_id AND day='".date('Y-m-d')."'),1), IFNULL((SELECT views FROM views WHERE file_id=$file_id AND day='".date('Y-m-d')."'),0), IFNULL((SELECT time_spent FROM views WHERE file_id=$file_id AND day='".date('Y-m-d')."'),0), NULL");
