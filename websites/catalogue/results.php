...
switch ($header_tab){
	case $cat_config['filmsoneshots_slug_internal']:
		$sections=array($cat_config['section_moviesoneshots']);
		$descriptions=array($cat_config['section_moviesoneshots_desc']);
		$queries=array(
			$base_query . " WHERE s.type='${cat_config['items_type']}' AND $query_portion_limit_to_non_hidden AND s.subtype='${cat_config['filmsoneshots_db']}'$cookie_extra_conditions GROUP BY s.id ORDER BY s.name ASC");
		$specific_version=array(FALSE);
		$type=array('static');
		break;
	case $cat_config['serialized_slug_internal']:
		$sections=array($cat_config['section_serialized']);
		$descriptions=array($cat_config['section_serialized_desc']);
		$queries=array(
			$base_query . " WHERE s.type='${cat_config['items_type']}' AND $query_portion_limit_to_non_hidden AND s.subtype='${cat_config['serialized_db']}'$cookie_extra_conditions GROUP BY s.id ORDER BY s.name ASC");
		$specific_version=array(FALSE);
		$type=array('static');
		break;
	case 'search':
...
?>
				<div class="hidden floating-info floating-info-pattern" onmouseenter="hoverFloatingInfo(this);" onmouseleave="leaveFloatingInfo(this);"></div>
