...
while ($row = mysqli_fetch_assoc($result)){
?>
						<div class="filter-fansub<?php echo (in_array($row['slug'], $selected_fansubs) ? ' filter-selected' : ''); ?>" id="filter-fansub-<?php echo $row['slug']; ?>"><img alt="" width="16" height="16" src="<?php echo !empty($row['id']) ? ('https://static.fansubs.cat/images/icons/'.$row['id'].'.png') : '/favicon.ico'; ?>" /><span><?php echo $row['name']; ?></span></div>
<?php
}
mysqli_free_result($result);
?>
