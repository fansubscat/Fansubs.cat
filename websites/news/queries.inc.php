...
function query_latest_news($user, $text, $page, $page_size, $fansub_id, $show_blacklisted_fansubs, $show_own_news, $min_month, $max_month) {
	...
	$final_query = "SELECT n.*, f.name fansub_name, IFNULL(f.slug,'fansubs-cat') fansub_slug, f.url fansub_url, f.archive_url
			...
			WHERE ".($text!==NULL ? "(n.title LIKE '%".escape($text)."%' OR n.contents LIKE '%".escape($text)."%')" : "1")."
				...
				AND ".($fansub_id>0 ? "(n.fansub_id IS NULL OR n.fansub_id=$fansub_id)" : "1")."
...
function query_all_fansubs_with_news() {
