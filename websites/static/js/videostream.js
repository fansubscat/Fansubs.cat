//The following code was added by Fansubs.cat so the videostream.js provided by Mega works
//The original file (bundle/videostream.js) is Copyright (c) John Hiesey with modifications
//done by Mega.nz and available under the MIT license at: https://github.com/meganz/videostream
//vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv

//Appears to be some debug variable, set to 0 to disable debug
//If set to high values, video playback does not work with some files
var d=0;

//onIdle: function to call certain functions when possible
if (window.requestIdleCallback) {
	var onIdle = function onIdle(cb) {
		return window.requestIdleCallback(cb, {timeout: 20});
	};
} else {
	//Simulate it
...
//Needed for certain UA checks
var useragent = navigator.userAgent.toLowerCase();
var ua = {};
ua.details = {};
if (useragent.indexOf('gecko') > 0) {
	ua.details.engine = 'Gecko';
	var match = window.navigator.userAgent.match(/Firefox\/([0-9]+)\./);
	ua.details.version = match ? parseInt(match[1]) : 0;
}

//Runs a function by surrounding it in a try/catch
function tryCatch(originalFn, errorCallback){
	tryCatchedFn = function() {
		try {
			return originalFn.apply(this, arguments);
		} catch (e) {
			if (errorCallback !== false) {
				errorCallback.bind(null, e);
			} else {
				console.error(e);
			}
		}
...
var delayedList = [];
function hasDelayed(id) {
    return delayedList[id] !== undefined;
};

function cancelDelayed(id) {
    if (hasDelayed(id)) {
        clearTimeout(delayedList[id].timeoutId);
        delete delayedList[id];
        return true;
    }
    return false;
};
function delay(id, fn, timeout) {
    var element = delayedList[id];
    if (!element) {
	element = {};
        element.id = id;
        element.timeoutId = setTimeout(function(element) {
            ...
            if (remainingTime < 50 || remainingTime*100/element.totalTimeout < 2) {
                ...
                delay(element.id, element.task, remainingTime);
            }
        }, timeout, element);
        delayedList[id] = element;
    }
    element.totalTimeout = timeout;
    element.task = fn;
    element.launchTime = new Date().getTime();

    return id;
}
delay.cancel = cancelDelayed;

//Later method: wait 1s
function later(cb) {
    return setTimeout(function() {
        cb();
    }, 1000);
...
//Lazy method: doing nothing does not appear to have an impact
function lazy(rootElem, property, fn) {
}

//Used only for debug, no need to implement it
function secondsToTime(seconds, otherParameter) {
	return null;
}
...
var MegaLogger = {};
MegaLogger.getLogger = function(name){
	return window.console;
}

//Do nothing: only used for audio
function SoonFc(ms, callback) {
}
...
M.gfsfetch = function(data, byteOffset, byteLength){
	...
	myAttributes.byteOffset=byteOffset;
	myAttributes.byteLength=byteLength;
	myAttributes.catch = function(catchFn) {
		myAttributes.catchFn = catchFn;
		return myAttributes;
	};
	myAttributes.then = function(then) {
		var start = myAttributes.byteOffset;
		...
		megaJsQueuedDownload(currentMegaFile, {'start': start, 'end': end, 'maxConnections': 1, 'initialChunkSize': chunkSize, 'chunkSizeIncrement': chunkSize, 'maxChunkSize': chunkSize}, (err, chunk) => {
			if (err) {
				...
				if (/EOVERQUOTA \(\-17\)/.test(err) || /Bandwidth limit reached/.test(err)) {
					myAttributes.catchFn({target: {status: 509}}, null);
				} else {
					myAttributes.catchFn(err, null);
				}
			} else {
				console.debug('Loaded chunk '+start+'-'+end);
				chunk.payload = { s: chunk.length };
				then(chunk);
			}
		});
		return myAttributes;
 ...
 * $Id: videostream.js,v 5.5.3 2025/05/03 10:10:07 dc Exp $
 */
(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
var inherits = require("/bundle/utils").inherit, EventEmitter = require("eventemitter3").EventEmitter, Buffer = require("buffer").Buffer;
...
function AudioStream(file, mediaElem, opts) {
  ...
  self._isWebkitSafari = window.safari || "Webkit" === ua.details.engine;
  var fileStream = file.createReadStream({
    start: 0
  });
  (self._fileStream = fileStream).on("data", tryCatch(function(data) {
    ...
    if (self._isWebkitSafari && !AudioStream.patched) {
      AudioStream.patched = !0;
      log("patching decode-audio-data if needed...");
      require("promise-decode-audio-data");
    }
    if (opts.partial) {
      self._feed(opts);
      var read = self._bytesRead || self._buffer.byteLength, size = Math.min(file.fetchChunkSize << 1, AudioStream.maxChunkSize);
      if (read > size) {
  ...
  "suspended" === context.state && self._resume();
  if (self._isWebkitSafari) {
    self._onVisibilityChange = function() {
      "suspended" !== context.state && "interrupted" !== context.state || self._resume();
    };
...
Object.defineProperty(AudioStream, "maxChunkSize", {
  value: 4194304
});

Object.defineProperty(AudioStream, "partialChunkSize", {
  value: 262144
});
...
AudioStream.prototype.destroy = function(err) {
  if (!this.destroyed) {
    ...
    this._onVisibilityChange && window.removeEventListener("visibilitychange", this._onVisibilityChange);
...
AudioStream.prototype._decode = function(opts) {
  ...
  if (self._buffer && !self._decoding) {
    ...
    if (done) {
      self.emit("audio-buffer", data = data.buffer);
      self._buffer = null;
    } else {
      if (this._bufTime > 20) {
        self._decoding = !0;
      ...
      try {
        data = data.buffer.slice(0, self._bytesRead || void 0);
      } catch (ex) {
        log(ex);
        return;
      }
    ...
    len = data.byteLength;
    d > 1 && log("Decoding new chunk...", done, len);
    self._decoding = !0;
    self._audioContext.decodeAudioData(data).then(function(buffer) {
      (done || !opts.startTime || buffer.duration > opts.startTime) && self._setup(opts.autoplay, opts.startTime, buffer);
    }).catch(function(ex) {
      (d && done || d > 1) && log(ex);
      done && self.destroy(ex);
      self._decTick = 1e11;
    }).finally(function() {
      self._decoding = !1;
      var c, b, again = !done && self._complete;
      !again && self._buffer && (c = self._buffer, again = ((b = self._bytesRead) > 0 ? Math.min(b, c.byteLength) : c.byteLength) !== len);
      again && self._decode(opts);
    });
  }
};
...
AudioStream.prototype._feed = function(opts) {
  var self = this;
  if (!self._decoding) return self._decode(opts);
...
AudioStream.prototype._setup = function(autoplay, time, buffer) {
  var elm = this._elem;
  if (this.destroyed) log("This instance has been destroyed.", [ this ]); else {
    ...
    if (this._audioBuffer) {
      ...
      d > 1 && log("new chunk arrived...", [ buffer ], buffer.duration, time);
      var o = this._stopOffset;
      o && o < this._audioBuffer.duration && (this._played = -1);
      this._audioBuffer = buffer;
      this._play(time);
      this.emit("duration", buffer.duration);
    } else {
      var audioContext = this._audioContext, audioStream = audioContext.createMediaStreamDestination(), videoCanvas = document.createElement("canvas"), videoContext = videoCanvas.getContext("2d"), videoStream = this._captureStream(videoCanvas), tracks = [ audioStream.stream.getTracks()[0], videoStream.getTracks()[0] ], audioAnalyser = audioContext.createAnalyser();
      d > 1 && log("setup", buffer, autoplay, tracks);
...
AudioStream.prototype._stop = function() {
  if (!this._stopping) {
    this._stopping = !0;
    var audioSource = this._audioSource, visualiser = this._visualiser, ctx = this._audioContext, pos = this.currentTime;
    if (audioSource) try {
    ...
    if (this._played) {
      this._timeOffset = pos;
      this._stopOffset = pos;
      this._playOffset = ctx.currentTime;
    }
    this._stopping = !1;
  }
};
...
AudioStream.prototype.__tryPlay = function(time) {
  var self = this, rate = self.playbackRate, context = self._audioContext, visualiser = self._visualiser, audioBuffer = self._audioBuffer;
  if (audioBuffer) {
    this._stop();
    if ("interrupted" !== context.state) {
    ...
    } else context.resume().then(function() {
      self._play(time);
    }).catch(dump);
...
AudioStream.prototype._setPBS = function(v) {
  ...
  delay("vs.audio.playback", function() {
    ...
    if (f && !self.destroyed) {
      var s = f.playing;
      f.paused = !v;
      (f.playing = !!v) ? self.emit("activity") : s && self.emit("inactivity");
    }
  });
};

AudioStream.prototype._resume = SoonFc(40, function() {
  Promise.resolve(this._audioContext.resume()).catch(dump);
});

AudioStream.prototype._endOfStream = tryCatch(function() {
  if (!this._ended) {
    var v = this._elem, dsp = function() {
      v.removeEventListener("pause", dsp);
      v.dispatchEvent(new Event("ended"));
    };
    this._ended = !0;
    if (v.paused) dsp(); else {
...
AudioStream.prototype._captureStream = function(canvas) {
  var stream, raf = !parseInt(localStorage.vsnraf);
  if (!raf) try {
  ...
  if (!stream) {
    raf || log("This browser does lack CanvasCaptureMediaStreamTrack.requestFrame");
...
Object.defineProperty(AudioStream.prototype, "_bufTime", {
  get: function() {
    return (this._audioBuffer || !1).duration - this.currentTime;
  }
});
...
Object.defineProperty(AudioStream.prototype, "playbackRate", {
  get: function() {
    ...
    return s && s.playbackRate && s.playbackRate.value || 1;
  },
  set: function(v) {
    var s = this._audioSource;
    if (s && s.playbackRate) {
...
Object.defineProperty(AudioStream.prototype, "currentTime", {
  get: function() {
    if (this._stopOffset || !this._played) return this._stopOffset;
    var audioBuffer = this._audioBuffer || !1, result = this._timeOffset + (this._audioContext.currentTime - this._playOffset) * this.playbackRate;
    if (result >= audioBuffer.duration) if (this.ready) {
      this._endOfStream();
...
AudioVisualiser.prototype._draw = function() {
  ...
  self._stop = function() {
    if (tick === this._tick++) {
      ...
      stream.requestFrame && stream.requestFrame();
      self._hasFocus || ++tick;
    }
  };
  !function _draw() {
...
Visualiser.prototype.init = function(ctx, width, height, canvas) {
  ...
  _gradient.addColorStop(1, "rgba(96, 96, 98, .3)");
  _gradient.addColorStop(.35, "rgba(74, 69, 74, .6)");
...
Visualiser.prototype.draw = function(ctx, width, height) {
  ...
  if (this._hasFocus) {
    ...
    i = data.byteLength;
    while (i--) {
      var x = i / data.byteLength * width, y = (255 - data[i]) / 255 * (.34 * height) + .66 * height;
      i === data.byteLength - 1 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    }
    ctx.stroke();
    ctx.closePath();
  }
...
},{"/bundle/utils":4,"buffer":10,"eventemitter3":15,"promise-decode-audio-data":26}],2:[function(require,module,exports){
var bcp47iso639={aar:"aa",abk:"ab",afr:"af",aka:"ak",alb:"sq",sqi:"sq",amh:"am",ara:"ar",arg:"an",arm:"hy",hye:"hy",asm:"as",ava:"av",ave:"ae",aym:"ay",aze:"az",bak:"ba",bam:"bm",baq:"eu",eus:"eu",bel:"be",ben:"bn",bih:"bh",bis:"bi",tib:"bo",bod:"bo",bos:"bs",bre:"br",bul:"bg",bur:"my",mya:"my",cat:"ca",cze:"cs",ces:"cs",cha:"ch",che:"ce",chi:"zh",zho:"zh",chu:"cu",chv:"cv",cor:"kw",cos:"co",cre:"cr",wel:"cy",cym:"cy",dan:"da",ger:"de",deu:"de",div:"dv",dut:"nl",nld:"nl",dzo:"dz",gre:"el",ell:"el",eng:"en",epo:"eo",est:"et",ewe:"ee",fao:"fo",per:"fa",fas:"fa",fij:"fj",fin:"fi",fre:"fr",fra:"fr",fry:"fy",ful:"ff",geo:"ka",kat:"ka",gla:"gd",gle:"ga",glg:"gl",glv:"gv",grn:"gn",guj:"gu",hat:"ht",hau:"ha",heb:"he",her:"hz",hin:"hi",hmo:"ho",hrv:"hr",hun:"hu",ibo:"ig",ice:"is",isl:"is",ido:"io",iii:"ii",iku:"iu",ind:"id",ipk:"ik",ita:"it",jav:"jv",jpn:"ja",kal:"kl",kan:"kn",kas:"ks",kau:"kr",kaz:"kk",khm:"km",kik:"ki",kin:"rw",kir:"ky",kom:"kv",kon:"kg",kor:"ko",kua:"kj",kur:"ku",lao:"lo",lat:"la",lav:"lv",lim:"li",lin:"ln",lit:"lt",ltz:"lb",lub:"lu",lug:"lg",mac:"mk",mkd:"mk",mah:"mh",mal:"ml",mao:"mi",mri:"mi",mar:"mr",may:"ms",msa:"ms",mlg:"mg",mlt:"mt",mon:"mn",nau:"na",nav:"nv",nbl:"nr",nde:"nd",ndo:"ng",nep:"ne",nno:"nn",nob:"nb",nor:"no",nya:"ny",oci:"oc",oji:"oj",ori:"or",orm:"om",oss:"os",pan:"pa",pli:"pi",pol:"pl",por:"pt",pus:"ps",que:"qu",roh:"rm",rum:"ro",ron:"ro",run:"rn",rus:"ru",sag:"sg",san:"sa",sin:"si",slo:"sk",slk:"sk",slv:"sl",sme:"se",smo:"sm",sna:"sn",snd:"sd",som:"so",sot:"st",spa:"es",srd:"sc",srp:"sr",ssw:"ss",sun:"su",swa:"sw",swe:"sv",tah:"ty",tam:"ta",tat:"tt",tel:"te",tgk:"tg",tgl:"tl",tha:"th",tir:"ti",ton:"to",tsn:"tn",tso:"ts",tuk:"tk",tur:"tr",twi:"tw",uig:"ug",ukr:"uk",urd:"ur",uzb:"uz",ven:"ve",vie:"vi",vol:"vo",wln:"wa",wol:"wo",xho:"xh",yid:"yi",yor:"yo",zha:"za",zul:"zu"},ns=Object.create(null);lazy(ns,"lang",function(){return String(localStorage.vslang||navigator.language||mega.intl.locale).split("-")[0]||"en"});ns.decodeMP4LangCode=function(code){return String.fromCharCode.apply(null,[96+(code>>10&31),96+(code>>5&31),96+(31&code)])};ns.bcp47match=function(lang){"number"===typeof lang&&(lang=ns.decodeMP4LangCode(lang));if((lang=bcp47iso639[lang])&&lang===(localStorage.vslang||$.vslang))return lang;for(var locales=["nn-"+mega.ipcc,mega.intl.locale,navigator.language].concat(navigator.languages),i=0;i<locales.length;++i){var locale=String(locales[i]).split("-");if(locale[1]&&String(locale[1]).toLowerCase()===lang)return lang;if(lang&&lang===locale[0])return lang}return!1};module.exports=ns;
...
CacheStream.prototype._read = function(size) {
  if (this._file && this._file._vs) {
    ...
    var videoFile = this._file, videoStream = videoFile._vs, fileSize = videoFile.filesize, currentTime = videoStream._elem.currentTime, bufTime = currentTime && !(videoFile.seeking || videoFile.streamer.playbackSeeking) && videoStream.bufTime;
...
function VideoFile(data, streamer) {
  ...
  this.mru = 0;
  this.curfetch = 0;
  this.filesize = currentMegaFile.size; //Changed Fansubs.cat
  ...
  if (data instanceof Blob) {
    ...
    if (o.filesize > 0) {
      ...
      if (o.bitrate > this.fetchChunkSize) {
        this.fetchChunkSize = o.bitrate + 16777216 & -16777216;
        d && console.debug("chunk-size set to %s", this.fetchChunkSize);
      }
    }
  }
...
VideoFile.prototype.fetcher = function(data, byteOffset, byteLength) {
  return SIMULATE_RESUME_FROM_STALLED && byteOffset > 8 * REQUEST_SIZE ? Promise.reject({
    target: {
      status: 509
    }
  }, data) : M.gfsfetch(data, byteOffset, byteLength);
};
...
VideoFile.prototype.fetch = function(startpos, recycle) {
  ...
  if (recycle && this.paused && this.cachesize >= this.minCache && this.canplay) d && console.debug("[VideoFile.fetch()] MIN_CACHE reached, will not fetch more data until no longer %s...", this.paused ? "paused" : "throttled"); else if (void 0 !== this.mru) {
    ...
    if (!(length < 1)) {
      ...
      this.fetcher(this.data, pos, pos + length).then(function(chunk) {
        var data = chunk.payload, buffer = chunk.buffer;
        if (void 0 !== self.mru) {
        ...
        var data = self.data, xhr = ev && ev.target || !1;
        if (void 0 !== self.mru) {
          delete self.fetching[pos];
          var retry = function() {
          ...
          if ("number" === typeof ev) {
            ...
            if (self.isOnline) {
              ...
              403 === xhr.status && "object" === typeof self.data && (self.data = self.data._ticket || self.data);
            } else self.retryq.push(retry);
          }
        }
      });
      return this;
 ...
 */ function Streamer(data, video, options) {
  ...
  this._events = [ "progress", "timeupdate", "canplay", "pause", "playing", "error", "abort", "updateend", "ended", "stalled", "suspend", "durationchange" ];
  ...
  if (d) {
    window.strm || (window.strm = new WeakSet());
...
Streamer.prototype.init = function(data) {
  ...
  if (self.video) {
    ...
    if (this.goAudioStream) {
      void 0 === options.partial && (options.partial = !localStorage.vsNoPartialAudio);
      if (options.partial) {
...
Streamer.prototype.initTypeGuess = function(data) {
  ...
  file.fetcher(data, 0, 16).then(function(chunk) {
    var dv = new DataView(chunk.buffer), long = dv.getUint32(0, !1);
    ...
    init("string" === typeof data ? chunk.payload || chunk : data);
  }).catch(function(ex) {
...
*/
  ...
  d && window.strm && window.strm.delete(this);
  delete this.evs;
  delete this.file;
  delete this.video;
  delete this.stream;
...
Streamer.prototype.onPlayBackEvent = function(playing) {
  ...
  if (playing) {
    ...
    this.playbackSeeking = !1;
    this.stream._sbQuotaError && (this.stream._sbQuotaError = !1);
...
Streamer.prototype.canSwitchAudioTrack = tryCatch(function(error) {
  var stream = this.stream || !1, muxer = stream._muxer || !1;
  if (stream instanceof VideoStream && muxer._chosenAudioTrack >= 0) {
    this.options.bat || (this.options.bat = Object.create(null));
    this.options.bat[muxer._chosenAudioTrack] = 1;
    d && error && console.info("Stream error, attempting to switch to another audio track...", error, this.options);
    var self = this;
    tryCatch(function() {
      self.stream.destroy();
    })();
...
Streamer.prototype.handleEvent = function(ev) {
  ...
  (d && "timeupdate" !== ev.type || d > 4) && console.debug("Event(%s)", ev.type, target, ev);
  this.state = ev.type;
  if (!videoFile) {
  ...
  switch (ev.type) {
   case "seeking":
    ...
    this.video.paused && queueMicrotask(this.play.bind(this));
   ...
   case "durationchange":
    this.duration > 0 && this.notify("duration", this.duration);
    break;
   ...
   case "canplay":
    videoFile.canplay = !0;
    !this.options.autoplay || videoFile.playing || this.playbackEvent && videoFile.paused || this.play();
    if (videoFile.fetchChunkSize > REQUEST_SIZE) {
      videoFile.fetchChunkSize = REQUEST_SIZE;
      d && console.debug("chunk-size reset to %s", videoFile.fetchChunkSize);
    }
    break;

   case "ended":
    this._clearActivityTimer();
    this.stream instanceof VideoStream && this.stream.flushSourceBuffers(-1);
    break;

   case "stalled":
    var stream = this.stream;
    if (!this.isOverQuota && stream instanceof VideoStream) if (videoFile.playing) stream.findTimeGAPs() && stream._enqueueForcePump(); else if (this.playbackSeeking) {
      d && console.warn("Forcing pump while stalled on seeking...", this.video.currentTime, [ this ]);
  ...
  if (this.evs[ev.type]) {
    if ("error" === ev.type) {
      ...
      if (this.canSwitchAudioTrack(error || -13)) return;
    }
...
Streamer.prototype.play = function() {
  try {
    ...
    "undefined" !== typeof Promise && promise instanceof Promise && promise.then(function() {
      ...
      var f = self.file || !1;
      if (f.canplay && ex && "NotAllowedError" === ex.name) {
        f.bgtask = -1;
        f.minCache = 4194304;
...
Streamer.prototype._setActivityTimer = function(force) {
  var self = this;
  !force && self.stream instanceof AudioStream || delay("vs:activity-timer", function() {
...
Streamer.prototype.getImage = function(w, h) {
  var tick = 96, self = this, video = this.video;
  return new Promise(function _(resolve, reject) {
    if (video.videoWidth && !video.error) {
      ...
      if (Math.round(100 * bp / len) > 70) {
        ...
        video.paused || video.ended || !--tick ? queueMicrotask(function() {
          reject(-5);
...
Streamer.getThumbnail = function(data) {
  return new Promise(function(resolve, reject) {
    if (data.__getVTNPid) return reject(-5);
    Object.isExtensible(data) && Object.defineProperty(data, "__getVTNPid", {
      value: Streamer.pid
    });
    ...
    var step = -1, offset = -1, tid = "vst." + makeUUID(), s = Streamer(data, video, {
      videoOnly: !0
    }), _onstalled = function() {
      var stream = s.stream;
      if (stream) {
        if (!(stream instanceof VideoStream)) return _reject(-9);
        queueMicrotask(_pump);
        offset < 1 && (offset = 0 | Math.min(2 + (0 | video.currentTime), video.duration - 3));
        if ((offset += 2.3) + 3 > video.duration) return _reject(-7);
        stream._enqueueForcePump(s.currentTime = offset);
        return !0;
      }
    };
    s.on("progress", _pump);
...
Object.defineProperty(Streamer.prototype, "ended", {
  get: function() {
    var stream = this.stream;
    return (this.video || !1).ended || stream instanceof AudioStream && stream._ended;
  }
});

Object.defineProperty(Streamer.prototype, "paused", {
  get: function() {
    return (this.video || !1).paused;
  }
});
...
Object.defineProperty(Streamer.prototype, "playbackRate", {
  get: function() {
    var video = this.video, stream = this.stream;
    return stream instanceof AudioStream ? stream.playbackRate : video && video.playbackRate;
  },
  set: tryCatch(function(v) {
    var video = this.video, stream = this.stream;
    stream instanceof AudioStream ? stream.playbackRate = v : video && (video.playbackRate = v);
  })
});
...
Object.defineProperty(Streamer.prototype, "duration", {
  get: function() {
    var video = this.video || !1;
    return Infinity !== video.duration && video.duration - this.presentationOffset;
  }
});
...
},{"../audiostream":1,"../videostream":46,"/bundle/utils":4,"buffer":10,"readable-stream":41}],4:[function(require,module,exports){
var Buffer = require("buffer").Buffer, onNextTick = function() {
  function dsp() {
    var i = 0, q = cbs;
    cbs = [];
  ...
  var cbs = [];
  return function(cb) {
    1 === cbs.push(cb) && queueMicrotask(dsp);
  };
}();
...
var DEBUG = 0 | localStorage.vsd, DEBUG_INFO = DEBUG || window.d, DEBUG_ALL = DEBUG > 3, DEBUG_TRACE = DEBUG > 2, DEBUG_VERBOSE = DEBUG > 1;
...
EBMLRemuxer.prototype.emitInitSegment = function(segment) {
  ...
  DEBUG_INFO && console.log("initSegment", segment, [ this ]);
  ...
  for (var getCodec = function(codec) {
    ...
    var track = tracks[i], codec = String(track.CodecID).replace(/\W/g, ""), type = track.TrackType, mime = function(codec, type) {
      switch (codec) {
       case "A_VORBIS":
       case "A_OPUS":
       case "V_VP8":
    ...
    DEBUG_INFO && console.debug("Track%s, %s, %s", track.TrackNumber, codec, mime, track);
    if (mime) {
      if (this._hasVideo || 1 !== type) {
        if (this._hasAudio || 2 !== type) continue;
        if (!MediaSource.isTypeSupported(mime)) {
          DEBUG_INFO && console.debug("Unsupported audio track.", mime);
          continue;
        }
...
EBMLReader.prototype._onFinish = function() {
  var v = -1, s = this._clusterOffset;
  if (s > 0) {
    var b = this._lastBlock, dur = b.data.readUInt32BE(0) / 1e6 | 0;
    this._addTimeCode(this._timecodes[1 & this._clusterCount] + dur);
    v = -dur;
    this._cluster = this._read(s, b.end);
  }
  null !== this._cluster && this._emitCluster(v);
  this.emit("cluster", null);
  this.destroy();
};

lazy(EBMLReader.prototype, "_clearDefaultDuration", function() {
  return !localStorage.vsncdd && void 0 !== window.chrome || localStorage.vscdd;
});

EBMLReader.prototype._onData = function(state, data) {
  if (DEBUG_VERBOSE && "SimpleBlock" !== data.name) {
    if ("start" === state) {
      this._conGroup++;
  ...
  if ("start" === state) {
    if (!this._byteOffset) if ("EBML" === data.name) this._ebmlOffset = data.start; else if ("Segment" === data.name) this._segmentOffset = -1; else if (this._segmentOffset < 0) this._segmentOffset = data.start; else if ("Cluster" === data.name) {
      ...
      if (dd && this._clearDefaultDuration) {
        DEBUG_INFO && console.debug("Cleaning DefaultDuration...\n" + hexdump(segment.data, dd.start).slice(0, 4).join("\n"));
    ...
    var tmp, name = data.name.toLowerCase(), C = "cluster" === name, E = C && -1 === data.end;
    ...
    if (data.end - data.start > 0 || E) {
      if (this.listenerCount(name)) {
        ...
        if (C) if (E) {
          ...
          s >= 0 && (this._cluster = this._read(s, data.start));
          this._clusterOffset = data.start;
        } else this._cluster = chunk; else {
          if ("cues" === name) {
    ...
    "SimpleBlock" === data.name || "Block" === data.name ? this._lastBlock = data : this._segment[data.name] = this._getValue(data);
    "Timecode" === data.name ? this._addTimeCode(this._getValue(data)) : "DefaultDuration" === data.name && (this._trackDefaultDuration = data);
  }
};
  ...
  this._async(function() {
    self._reader = muxer._createReader(offset, self, "cluster", function(cluster) {
      if (!self.destroyed) if (cluster) self.append.apply(self, arguments); else {
        DEBUG_INFO && console.debug(self + " EOS", null === cluster);
        self.push(null);
        self.destroy();
      }
    });
...
MediaSegment.prototype.append = function(cluster, timecode, duration) {
  if (duration < 0) {
    this.playtime || (this.playtime = (timecode + -duration) * this.timescale);
    duration = this.playtime / this.timescale - timecode;
    DEBUG_INFO && console.debug(this + " Last cluster, duration:", duration);
  }
  var timecodeOffset = -1, cs = cluster.readUInt32BE(4);
  if (16777216 === cs) timecodeOffset = 12; else {
    if (33554431 === cs || !this.segment.playtime) {
      DEBUG_INFO && console.warn("non-seekable cluster, 0x%s", cluster.toString("hex", 0, 32));
  ...
  if (timecodeOffset > 0) {
    if ((cs = cluster.readUInt16BE(timecodeOffset)) >> 8 === 191) {
      DEBUG_INFO && console.debug(this + " mkv timecode fixup (0x%s)", cluster.toString("hex", 0, timecodeOffset + 9));
    ...
    } else DEBUG_INFO && console.warn("Unexpected timecode offset...");
  } else if (DEBUG_INFO) {
...
function createStream(dest, source, destroy) {
  ...
  dest.prototype.$etup = function() {
    ...
    if (DEBUG_VERBOSE) {
      ...
      this.emit = function(event) {
        "data" === event && !DEBUG_ALL || console.warn(this + ".emit(%s)", event, arguments, [ this ]);
  ...
  dest.prototype.destroy = function(err) {
    DEBUG_INFO && console[err ? "error" : "debug"](this + ".destroy", this.destroyed, [ this ], err);
    if (!this.destroyed) {
      this.destroyed = !0;
...
},{"/bundle/utils":4,"buffer":10,"ebml/lib/ebml/decoder":11,"eventemitter3":15,"readable-stream":41}],6:[function(require,module,exports){
var bs = require("binary-search"), EventEmitter = require("eventemitter3").EventEmitter, inherits = require("/bundle/utils").inherit, mp4 = require("mp4-stream"), Box = require("mp4-box-encoding"), RangeSliceStream = require("range-slice-stream"), Buffer = require("buffer").Buffer, locale = require("./bundle/locale"), FIND_MOOV_SEEK_SIZE = 4096, SILENT_PLAYBACK = parseInt(localStorage.vsna);
...
function MP4Remuxer(file, opts) {
  ...
  this._file = file;
  this._decoder = null;
  this._videoOnly = opts && opts.videoOnly || SILENT_PLAYBACK;
  this._bogusAudioTracks = opts && opts.bat || Object.create(null);
  vsNT(this._findMoov.bind(this, 0));
}
...
MP4Remuxer.prototype._processTracks = function(traks) {
  function validateAudioTrack(trk) {
    if (!trk) return !1;
    var mime = trk.mime;
    if (!MediaSource.isTypeSupported(mime)) {
      if ("mp3" !== trk.codec || !MediaSource.isTypeSupported(mime = "audio/mpeg")) {
    ...
    if (self._hasVideo && hevc[self._hasVideo.type]) {
      ...
      if ("mp4a.a9" === trk.codec || !MediaSource.isTypeSupported(mime)) {
        d && console.warn("[hevc] unsupported audio track.", mime);
  ...
  for (var mime, codec, self = this, audio = [], hevc = Object.assign(Object.create(null), {
    hvc1: 1,
    hev1: 1
  }), mp3a = Object.assign(Object.create(null), {
    "mp4a.6b": 1,
    "mp4a.69": 1
  }), vide = Object.assign(Object.create(null), {
    avc1: 1,
    av01: 1,
    vp09: 1
  }, hevc), swap = Object.assign(Object.create(null), {
    fLaC: "flac",
    Opus: "opus",
    ".mp3": "mp3"
  }), i = 0; i < traks.length; i++) {
    var trak = traks[i], mdia = trak.mdia || !1, stsd = trak.mdia.minf.stbl.stsd.entries[0], lang = Object(mdia.mdhd).language, type = Object(mdia.hdlr).handlerType;
    if ("vide" === type && vide[stsd.type]) {
      codec = stsd.type;
      stsd.avcC ? codec += "." + stsd.avcC.mimeCodec : stsd.av1C ? codec = stsd.av1C.mimeCodec || codec : stsd.vpcC ? codec = stsd.vpcC.mimeCodec || codec : stsd.hvcC && (codec += stsd.hvcC.mimeCodec);
      mime = 'video/mp4; codecs="' + codec + '"';
      d && console.debug("Track%d: %s %s", i, locale.decodeMP4LangCode(lang), mime);
      self._hasVideo || (self._hasVideo = {
        type: stsd.type,
        codec: codec,
        mime: mime,
        trak: trak,
    ...
    } else if ("soun" === type && !self._videoOnly && !self._bogusAudioTracks[i]) {
      if ("mp4a" === (codec = stsd.type)) {
        codec = "mp4a";
        stsd.esds && stsd.esds.mimeCodec && (codec += "." + stsd.esds.mimeCodec);
        mp3a[codec] && (codec = "mp3");
      }
  ...
  if (audio.length < 2) self._hasAudio = validateAudioTrack(audio[0]); else {
    for (var eng, def, und, j = 0; j < audio.length; ++j) {
      var trk = validateAudioTrack(audio[j]);
      if (trk) {
        var lng = locale.decodeMP4LangCode(trk.lang);
        eng || "eng" !== lng || (eng = trk);
        !def && locale.bcp47match(lng) && (def = trk);
        und = und || trk;
      }
    }
    self._hasAudio = def || eng || und || !1;
  }
};

MP4Remuxer.prototype._processMoov = function(moov) {
  var traks = moov.traks || !1;
  ...
  this._hasVideo = !1;
  this._hasAudio = !1;
  this._processTracks(traks);
  d && console.debug("video%s:%s, audio%s:%s", this._hasVideo.idx, this._hasVideo.codec, 0 | this._hasAudio.idx, this._hasAudio.codec || "N/A");
  for (var i = 0; i < traks.length; i++) {
    var mime, trak = traks[i], stbl = trak.mdia.minf.stbl, stco = stbl.stco || stbl.co64;
    d && console.debug("handler=%s, type=%s, trak:", trak.mdia.hdlr.handlerType, stbl.stsd.entries[0].type, trak);
    if (this._hasVideo && this._hasVideo.trak === trak) {
      mime = this._hasVideo.mime;
      this._hasVideo = this._hasVideo.codec;
    } else {
      if (!this._hasAudio || this._hasAudio.trak !== trak) continue;
      mime = this._hasAudio.mime;
      this._chosenAudioTrack = this._hasAudio.idx;
      this._hasAudio = this._hasAudio.codec;
    }
    ...
    if (decodingTimeEntry.value) {
      while (1) {
        ...
        var size = stbl.stsz.sample_size || stbl.stsz.entries[sample], duration = decodingTimeEntry.value && decodingTimeEntry.value.duration || 0, sync = !0;
        ...
        if (++sample >= stbl.stsz.sample_count) break;
        offsetInChunk += size;
...
MP4Remuxer.prototype.seek = function(time) {
  ...
  highWaterMark = Math.min((highWaterMark + 16777216 & -16777216) >>> 0, Math.pow(2, 26) * self._tracks.length);
  return self._tracks.map(function(track, i) {
    track.outStream && track.outStream.destroy();
    if (track.inStream) {
...
Box.boxes.vpcC.decode = function(buf, offset, end) {
  ...
  var tmp = readUint8();
  this.version = 127 & tmp;
  if (1 !== this.version) console.warn("Unsupported/deprecated vpcC version %d.", this.version); else {
    p += 3;
    r.profile = readUint8();
    r.level = readUint8();
    tmp = readUint8();
    ...
    r.videoFullRangeFlag = 1 & tmp;
    r.buffer = Buffer.from(buf);
    r.mimeCodec = [ "vp09", ("0" + r.profile).slice(-2), ("0" + r.level).slice(-2), ("0" + r.bitDepth).slice(-2) ].join(".");
  }
  return r;
};
...
Box.boxes.hvcC.decode = function(buf, offset, end) {
  ...
  buf = buf.slice(offset, end);
  var tmp = readUint8();
  this.version = 255 & tmp;
  r.configurationVersion = this.version;
  tmp = readUint8();
  r.profile_space = tmp >> 6;
  r.tier_flag = (32 & tmp) >> 5;
  r.profile_idc = 31 & tmp;
  r.profile_compatibility_indications = function() {
    var v = buf.readUInt32BE(p);
    p += 4;
    return v;
  }();
  r.constraint_indicator_flags = function() {
    for (var out = new Uint8Array(6), i = 0; i < 6; i++) out[i] = readUint8();
    return out;
  }();
  r.level_idc = readUint8();
  r.min_spatial_segmentation_idc = 4095 & readUint16();
  r.parallelismType = 3 & readUint8();
  r.chromaFormat = 3 & readUint8();
  r.bitDepthLumaMinus8 = 7 & readUint8();
  r.bitDepthChromaMinus8 = 7 & readUint8();
  r.avgFrameRate = readUint16();
  tmp = readUint8();
  r.constantFrameRate = tmp >> 6;
  r.numTemporalLayers = (13 & tmp) >> 3;
  r.temporalIdNested = (4 & tmp) >> 2;
  r.lengthSizeMinusOne = 3 & tmp;
  r.buffer = Buffer.from(buf);
  var mime = ".";
  r.profile_space && (mime += String.fromCharCode(64 + r.profile_space));
  mime += r.profile_idc + ".";
  var j = tmp = 0, cpl = r.profile_compatibility_indications;
  while (1) {
    tmp = 1 & cpl;
    if (++j > 30) break;
    tmp <<= 1;
  ...
  for (j = 5; j >= 0; j--) (cpl[j] || tmp) && (tmp = "." + ("0" + Number(cpl[j]).toString(16)).slice(-2) + tmp);
...
Box.boxes.stsz.decode = function(buf, offset) {
  var size = (buf = buf.slice(offset)).readUInt32BE(0), num = buf.readUInt32BE(4), entries = [];
  if (0 === size) {
    entries = new Array(num);
    for (var i = 0; i < num; i++) entries[i] = buf.readUInt32BE(4 * i + 8);
  }
  return {
    sample_size: size,
    sample_count: num,
...
},{"./bundle/locale":2,"/bundle/utils":4,"binary-search":9,"buffer":10,"eventemitter3":15,"mp4-box-encoding":20,"mp4-stream":23,"range-slice-stream":28}],7:[function(require,module,exports){
...
var Buffer = function(a, b, c) {
  return "number" == typeof a ? allocUnsafe(a) : from(a, b, c);
}, base64 = require("base64-js"), ieee754 = require("ieee754");
...
Buffer.prototype.copy = function(target, targetStart, start, end) {
  ...
  var len = end - start;
  this === target ? this.copyWithin(targetStart, start, end) : Uint8Array.prototype.set.call(target, this.subarray(start, end), targetStart);
  return len;
};
...
function fromArrayView(a) {
  return isInstance(a, Uint8Array) ? fromArrayBuffer((a = new Uint8Array(a)).buffer, a.byteOffset, a.byteLength) : fromArrayLike(a);
}
},{"base64-js":8,"ieee754":16}],11:[function(require,module,exports){
...
},{"./schema.js":12,"./tools.js":13,"/bundle/utils":4,"buffer":10,"readable-stream":41}],12:[function(require,module,exports){
...
module.exports = eos;
},{"once":25}],15:[function(require,module,exports){
"use strict";
...
},{"/bundle/utils":4,"readable-stream":41,"to-arraybuffer":43}],18:[function(require,module,exports){
...
},{"./descriptor":19,"./index":20,"buffer":10,"uint64be":44}],19:[function(require,module,exports){
...
},{"buffer":10}],20:[function(require,module,exports){
...
},{"./boxes":18,"buffer":10,"uint64be":44}],21:[function(require,module,exports){
...
},{"/bundle/utils":4,"buffer":10,"mp4-box-encoding":20,"next-event":24,"readable-stream":41}],22:[function(require,module,exports){
...
},{"/bundle/utils":4,"buffer":10,"mp4-box-encoding":20,"readable-stream":41}],23:[function(require,module,exports){
...
},{"wrappy":45}],26:[function(require,module,exports){
"use strict";
...
},{"base-audio-context":7}],27:[function(require,module,exports){
...
module.exports = pump;
},{"end-of-stream":14,"once":25}],28:[function(require,module,exports){
"use strict";
...
var _require = require("readable-stream"), Writable = _require.Writable, PassThrough = _require.PassThrough, RangeSliceStream = function(_Writable) {
  function RangeSliceStream(offset, opts) {
    var _this;
    void 0 === opts && (opts = {});
    (_this = _Writable.call(this, opts) || this).satisfied = !0;
    _this._queue = [];
...
},{"/bundle/utils":4,"readable-stream":41}],29:[function(require,module,exports){
"use strict";
...
},{"/bundle/utils":4}],30:[function(require,module,exports){
"use strict";
...
},{"./_stream_readable":32,"./_stream_writable":34,"/bundle/utils":4}],31:[function(require,module,exports){
"use strict";
...
},{"./_stream_transform":33,"/bundle/utils":4}],32:[function(require,module,exports){
"use strict";
...
Readable.prototype.wrap = function(stream) {
  var i, _this = this, state = this._readableState, paused = !1;
  ...
  for (i in stream) void 0 === this[i] && "function" === typeof stream[i] && (this[i] = function(method) {
    return function() {
...
},{"../errors":29,"./_stream_duplex":30,"./internal/streams/buffer_list":35,"./internal/streams/destroy":36,"./internal/streams/state":39,"./internal/streams/stream":40,"/bundle/utils":4,"buffer":10,"eventemitter3":15,"string_decoder/":42}],33:[function(require,module,exports){
"use strict";
...
},{"../errors":29,"./_stream_duplex":30,"/bundle/utils":4}],34:[function(require,module,exports){
"use strict";
...
},{"../errors":29,"./_stream_duplex":30,"./internal/streams/destroy":36,"./internal/streams/state":39,"./internal/streams/stream":40,"/bundle/utils":4,"buffer":10}],35:[function(require,module,exports){
"use strict";
...
module.exports = eos;
},{"../../../errors":29,"once":25}],38:[function(require,module,exports){
"use strict";
...
},{"../../../errors":29,"./end-of-stream":37,"once":25}],39:[function(require,module,exports){
"use strict";
...
},{"../../../errors":29}],40:[function(require,module,exports){
module.exports = require("eventemitter3").EventEmitter;
},{"eventemitter3":15}],41:[function(require,module,exports){
...
},{"./lib/_stream_duplex.js":30,"./lib/_stream_passthrough.js":31,"./lib/_stream_readable.js":32,"./lib/_stream_transform.js":33,"./lib/_stream_writable.js":34,"./lib/internal/streams/end-of-stream.js":37,"./lib/internal/streams/pipeline.js":38}],42:[function(require,module,exports){
"use strict";
...
},{"buffer":10}],44:[function(require,module,exports){
...
var pump = require("pump"), MP4Remuxer = require("./mp4-remuxer"), EBMLRemuxer = require("./ebml-remuxer"), MediaElementWrapper = require("mediasource");
...
function VideoStream(file, mediaElem, opts) {
  ...
  "none" !== self._elem.preload && self._createMuxer(opts);
  self.flushSourceBuffers = opts.sbflush ? self._flushSourceBuffers : function() {
    if (self._sbQuotaError) {
      var s = file.streamer;
      if (s && "stalled" === s.state) {
        d && console.warn("Forcing pump on quota-error while stalled...");
        self._enqueueForcePump(Math.max(.3, self._elem.currentTime - 1.2));
      }
    }
  };
  self._onError = function(err) {
...
VideoStream.prototype._createMuxer = function(opts) {
  ...
  self._muxer = new ({
    WebM: 1,
    Matroska: 1
  }[self._type] ? EBMLRemuxer : MP4Remuxer)(self._file, opts);
...
VideoStream.prototype.error = function(err) {
  ...
  if (ew) {
    var ms = ew._mediaSource, sb = ms && ms.sourceBuffers, cb = ew.error.bind(ew, err);
    later(cb);
    if (sb && sb.length) for (var i = sb.length; i--; ) this.onSourceBufferUpdateEnded(sb[i], cb);
  }
};

VideoStream.prototype.createWriteStream = function(obj) {
  var videoStream = this, videoFile = videoStream._file, mediaSource = videoStream._elemWrapper.createWriteStream(obj), csb = mediaSource._createSourceBuffer, mediaSourceDestroy = mediaSource.destroy, trace = localStorage.vsd ? function(time, sb, chunk) {
    for (var start = time, end = time + chunk.duration, b = [], i = 0; i < sb.buffered.length; ++i) b.push(sb.buffered.start(i) + "-" + sb.buffered.end(i));
    console.debug("ranges(%s), timecode=%s, duration=%s, time=%s, start=%s(%s), end=%s(%s)", b.join(", "), chunk.timecode, chunk.duration, time, start, sb.appendWindowStart, end, sb.appendWindowEnd, sb.timestampOffset, chunk.seektime);
  } : null;
...
*/
  ...
  mediaSource._flow = function() {
    if (this._cb && !this.destroyed) {
      var sb = this._sourceBuffer;
      if (sb && !sb.updating) {
        if (videoFile.throttle && "open" === this._mediaSource.readyState && sb.buffered.length && sb.buffered.end(sb.buffered.length - 1) - this._elem.currentTime > this._bufferDuration) return;
        var cb = this._cb;
        this._cb = null;
        cb();
      }
...
VideoStream.prototype._pump = function(time) {
  try {
    ...
    "WebM" === this._type && this.withinBufferedRange(time + 1) ? d && console.debug("Ignoring pump within buffered range.", time) : this._tryPump(time);
  } catch (ex) {
    this.error(ex);
  }
};
...
VideoStream.prototype.destroy = function() {
  if (!this.destroyed) {
    ...
    if (this._tracks) {
      for (var track, tracks = this._tracks, destroy = function(track, instance) {
        try {
          track[instance] && track[instance].destroy();
        } catch (ex) {
          console.warn(track, instance, ex);
        }
      }, i = tracks.length; i--; ) {
        destroy(track = tracks[i], "mediaSource");
        destroy(track, "muxed");
      }
      this._tracks = !1;
    }
...
VideoStream.prototype.forEachSourceBuffer = function(cb) {
  if (this._tracks) for (var sb, startRange, endRange, ms, src, currentTime = this._elem.currentTime, i = this._tracks.length; i--; ) {
    ms = (src = this._tracks[i].mediaSource || !1)._mediaSource;
...
VideoStream.prototype.onSourceBufferUpdateEnded = function(sb, cb) {
  if (!sb.updating) return queueMicrotask(cb);
...
VideoStream.prototype._enqueueForcePump = function(offset) {
  if (++this._fpTick > 3) return this._forcePump(offset);
...
VideoStream.prototype._forcePump = function(offset) {
  function pump() {
    var f = self._file;
    if (!--step && f) {
      d && self.forEachBufferedRanges(dump);
      if (f.playing) {
        f.throttle = 0;
        self._pump(offset);
      }
  ...
  var self = this, res = 0, step = 0;
  self._fpTick = 0;
  delay.cancel("vs.force-track-pump");
  if (this._tracks) for (var ms, sb, t = this._tracks, i = t.length; i--; ) if ((sb = (ms = t[i].mediaSource) && ms._sourceBuffer) && ms) {
    ++step;
    ms._cb = null;
    ms.destroy();
    if (sb.updating) {
...
VideoStream.prototype.removeBuffered = function(sb, currentTime, callback) {
  if (sb.buffered.length) {
    ...
    tryCatch(sb.remove.bind(sb, startRange, endRange), d > 2 && dump)();
    callback && this.onSourceBufferUpdateEnded(sb, callback);
    return !0;
  }
...
VideoStream.prototype.forEachBufferedRanges = tryCatch(function(cb) {
  if (this._tracks) for (var j, sb, startRange, endRange, ms, src, currentTime = this._elem.currentTime, trackId = this._tracks.length; trackId--; ) {
    ms = (src = this._tracks[trackId].mediaSource || !1)._mediaSource;
    for (j = (sb = src._sourceBuffer) && sb.buffered.length; j--; ) try {
...
},{"./ebml-remuxer":5,"./mp4-remuxer":6,"mediasource":17,"pump":27}]},{},[3]);
