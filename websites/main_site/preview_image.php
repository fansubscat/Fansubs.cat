...
if (empty($failed)) {
	...
	$cover = imagecreatefromjpeg("images/series/$id.jpg");
	...
	//Type
	if ($series['type']=='movie' && $series['episodes']>1) {
		$text = "Anime • Conjunt de ".$series['episodes']." films";
	} else if ($series['type']=='movie') {
		$text = "Anime • Film";
	} else if ($series['seasons']>1) {
		$text = "Anime • Sèrie • ".$series['seasons']." temporades • ".($series['episodes']==-1 ? 'En emissió' : $series['episodes'].' capítols');
	} else {
		$text = "Anime • Sèrie • ".($series['episodes']==-1 ? 'En emissió' : $series['episodes'].' capítols');
	}
	...
	echo "Aquesta sèrie no existeix.";
}
