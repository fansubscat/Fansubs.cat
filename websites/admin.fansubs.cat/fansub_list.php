...
?>
					<table class="table table-hover table-striped">
						...
						<tbody>
<?php
	$result = query("SELECT f.id, f.name, f.url, f.status, f.historical, f.twitter_url, f.archive_url, (SELECT COUNT(*) FROM news WHERE fansub_id=f.id) news, COUNT(DISTINCT vf.version_id) anime_versions, COUNT(DISTINCT mvf.manga_version_id) manga_versions FROM fansub f LEFT JOIN rel_version_fansub vf ON f.id=vf.fansub_id LEFT JOIN rel_manga_version_fansub mvf ON f.id=mvf.fansub_id GROUP BY f.id ORDER BY f.status DESC, f.name ASC");
