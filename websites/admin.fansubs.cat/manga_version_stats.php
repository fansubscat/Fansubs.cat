...
<?php
	$result = query("SELECT IFNULL(SUM(clicks),0) total_clicks, IFNULL(SUM(views),0) total_views, IFNULL(SUM(pages_read),0) total_pages_read FROM manga_views v LEFT JOIN file fi ON v.file_id=fi.id WHERE fi.manga_version_id=".escape($_GET['id']));
	$totals = mysqli_fetch_assoc($result);
	mysqli_free_result($result);
?>
					<div class="row">
						<div class="col-sm-4 text-center"><b>Lectures reals:</b> <?php echo $totals['total_views']; ?></div>
						<div class="col-sm-4 text-center"><b>Clics sense llegir:</b> <?php echo max(0, $totals['total_clicks']-$totals['total_views']); ?></div>
						<div class="col-sm-4 text-center"><b>Pàgines llegides:</b> <?php echo $totals['total_pages_read']; ?></div>
					</div>
				</article>
			</div>
		</div>
...
<?php
	...
	$result = query("SELECT DATE_FORMAT(v.day,'%Y-%m') month, GREATEST(IFNULL(SUM(clicks),0)-IFNULL(SUM(views),0),0) total_clicks, IFNULL(SUM(views),0) total_views, IFNULL(SUM(pages_read),0) total_pages_read FROM manga_views v LEFT JOIN file fi ON v.file_id=fi.id WHERE fi.manga_version_id=".escape($_GET['id'])." GROUP BY DATE_FORMAT(v.day,'%Y-%m') ORDER BY DATE_FORMAT(v.day,'%Y-%m') ASC");
...
<?php
	...
	$result = query("SELECT DATE_FORMAT(v.day,'%Y-%m-%d') day, GREATEST(IFNULL(SUM(clicks),0)-IFNULL(SUM(views),0),0) total_clicks, IFNULL(SUM(views),0) total_views, IFNULL(SUM(pages_read),0) total_pages_read FROM manga_views v LEFT JOIN file fi ON v.file_id=fi.id WHERE fi.manga_version_id=".escape($_GET['id'])." GROUP BY DATE_FORMAT(v.day,'%Y-%m-%d') ORDER BY DATE_FORMAT(v.day,'%Y-%m-%d') ASC");
	while ($row = mysqli_fetch_assoc($result)) {
