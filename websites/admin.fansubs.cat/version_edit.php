...
if (!empty($_SESSION['username']) && !empty($_SESSION['admin_level']) && $_SESSION['admin_level']>=1) {
	if (!empty($_POST['action'])) {
		...
		if ($_POST['action']=='edit') {
			...
			$completed_on = 'NULL';
			if ($data['status']==1) {
				$result_old=query("SELECT completed_on FROM version WHERE id=".$data['id']);
