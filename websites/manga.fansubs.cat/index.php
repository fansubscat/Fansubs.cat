...
switch ($header_tab){
	...
	default:
		...
		if ($is_fools_day) {
			...
			//Worst rated completed or semi completed mangas
			...
			//Best rated completed and featurable mangas of genres Romance, Boys Love and Girls Love
			$sant_jordi_result = query("SELECT DISTINCT vr.id FROM manga_version vr LEFT JOIN manga sr ON vr.manga_id=sr.id WHERE (sr.rating<>'XXX' OR sr.rating IS NULL) AND vr.status=1 AND vr.is_featurable=1 AND sr.score IS NOT NULL AND vr.chapters_missing=0 AND sr.id IN (SELECT rsg.manga_id FROM rel_manga_genre rsg WHERE rsg.genre_id IN (7,23,38)) ORDER BY sr.score DESC LIMIT 10");
