...
if %errorlevel% equ 0 (
	...
	if "%subtitle_stream:~-4%" == ".ass" (
		rem ASS subtitles
		set filter_opts=ass="%subtitle_stream%"
	) else (
		rem SRT or other subtitles - will use Arial font
		set filter_opts=subtitles="%subtitle_stream%":force_style='Fontname=Arial,Fontsize=20'
	)
)
...
call :generate_streaming "%input_file%" %video_track% %audio_track% "%input_subs%" %video_action% %audio_action% "%input_file:~0,-4%"
exit /b 0
